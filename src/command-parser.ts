import type { ParsedCommand, ParsedFunctionCommand, ParsedPropertyCommand, CssSelectorCommand } from "./types"
...
export class CommandParser {
     ...
     */
    private static parseCommandArray(commandArrayString: string): ParsedFunctionCommand[] {
        const result: ParsedFunctionCommand[] = []
    ...
    static parseCommand(commandString: string): ParsedCommand[] {
        const parsedOrderedCommand: ParsedCommand[] = []
        ...
        for (const command of commandMethods) {
            if (command.includes(").")) {
                ...
                parsedOrderedCommand.push({
                    type: "property",
                    name: command,
                } as ParsedPropertyCommand)
            } else {
                // Assumed to be a CSS selector
                parsedOrderedCommand.push({
                    type: "css-selector",
                    selector: command,
                } as CssSelectorCommand)
            }
        }
