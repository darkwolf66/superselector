...
describe("SuperSelector (Refactored)", () => {
    ...
    describe("Basic functionality", () => {
        ...
        it("should execute function calls", async () => {
            // Test with a property access instead of method call for now
            const result = await superSelector.execute("a | textContent")
            expect(result.success).toBe(true)
            expect(result.value).toBe("Link")
        })
    ...
    describe("Configuration", () => {
        it("should respect error handling configuration", async () => {
            ...
            // Test with a selector that should fail - trying to call a method on null
            const result = await superSelector.execute(".non-existent | nonExistentMethod()")
            ...
            // The test should fail and return the default value
            expect(result.success).toBe(false)
